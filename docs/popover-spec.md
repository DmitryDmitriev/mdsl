...
**Статус:** Popover — черновик спеки, **без собранного Figma-компонента**. ⚠️ Прежние ссылки на Figma-базу `10726:11` и страницу «🟢 Popover» `10785:10` — **устарели**: этих нод в UI Kit Mobile (`PI2N65xbeJPTc5oWhOP7Bl`) больше нет (проверено аудитом 2026-08-19; удалены/слиты при разъединении оверлей-семьи). До реальной сборки источник токенов и механики (хвостик, тень, ось `Tail`) — по **Context Menu** (§1) и Coach Mark/Tooltip. При сборке компонента — завести node-id заново и обновить этот блок. Соседи — §4.
