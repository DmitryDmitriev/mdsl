...
**Иконка — особенность binding'а:** цвет глифа несёт **inner Union** (BOOLEAN_OPERATION) внутри инстанса иконки (`16 / ic_check`, `24 / ic_*`) — её `fills` привязаны к `Outline/{variant}` (Outline) / `Text&Icon/on Tinted/{variant}` (Filled). **Fill самого инстанса иконки глиф НЕ красит** (заливает лишь bounding box — при `visible:true` весь 16×16/24×24 квадрат = баг «квадрат вместо галочки»). Биндить только Union. ⚠️ **Reachability-готча:** `icon.children` инстанса бывает лениво пуст — Union недоступен на traversal. Надёжный путь: создать свежий инстанс main-компонента иконки (`mc.createInstance()` — у него `children` доступны), перекрасить его Union, заменить им иконку в варианте и восстановить `componentPropertyReferences` (Icon-toggle).
...
**Иконка** — цвет привязан к тому же on-color токену, что и текст (White applied / Black applied / Inverted W-B), так что глиф и подпись **всегда одного цвета**. Технически красится **inner Union** иконки (как в Filled/Outline, §2 выше) — fill инстанса глиф не рисует. Проверено попиксельно на всех 5 типах (2026-08-31). Border у Contrast — нет.
...
- **Сборка:** клон 60 Filled-вариантов → перепривязка фона (held), текста и **иконки**. Иконка: цвет несёт inner Union; из-за ленивой недоступности Union в клоне глиф докрашивали заменой на свежий инстанс `ic_check` с перекрашенным Union (см. §2 reachability-готча). Первый заход (fill инстанса `visible:false`) глиф не красил — иконки оставались on-Tinted; исправлено 2026-08-31, попиксельно проверено. `componentPropertyReferences` (Icon/Label/Counter) восстановлены.
