...
| Warning  | Amber/50 | Amber/900 | Badge, soft-кнопки, информационные плашки в карточках |
...
> ✅ **Warning унифицирован на Amber (гармонизация PB-1583, Figma — 2026-07-03).**
>
> Все роли Warning теперь на **Amber**-шкале: Filled (`Background/Tinted/Warning` = Amber/50 Light / Amber/900 Dark; `Text&Icon/on Tinted/Warning` = Amber/800 Light / Amber/50 Dark), Outline (`Outline/Warning` = Amber/700/400), Accent (`Accent/Warning` = Amber/400), Border (`Border/Warning` = Amber/400). Amber-шкала в палитре полная (50…950), новые shade'ы не потребовались.
>
> **История drift'а:** Filled Warning исторически сидел на **Orange** (Orange/50/800/900), а Outline/Accent — на Amber (при вводе `Outline/*` 2026-05-26, commit `249a0ef`, взяли Amber/700 как Tailwind-default, без сверки с Filled). WCAG проходило, поэтому несоответствие заметили только на dev-reconciliation 2026-05-29. Зафиксировано как historical drift, отмашка на гармонизацию дана 2026-07-01, выполнено в Figma 2026-07-03 (ре-алиас двух семантических токенов Orange→Amber в `App Color Palette`).
>
> **Orange** освобождён — используется только в `Decor/Orange` (§1.6) для marketing-меток; смешения semantic и decor больше нет.
>
> **Остаётся (dev-side, PB-1583):** переиздать библиотеку `App Color Palette` → обновить UI-Kit; синк значений на iOS / Android. Компоненты (Badge Filled-Warning, Alert, Snackbar) наследуют семантические токены — правок в мастерах не требуется, но после публикации свериться визуально.
