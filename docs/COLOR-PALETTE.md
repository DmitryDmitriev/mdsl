...
> **Warning живёт на двух hue (Orange + Amber) — осознанный выбор, не баг.** Filled (`Background/Tinted/Warning` + `Text&Icon/on Tinted/Warning`) использует **Orange**-шкалу (мягкий peach-фон + тёмно-коричневый текст). Outline (`Outline/Warning`) и Accent (`Accent/Warning`) — **Amber**-шкалу (яркий янтарный для контраста и сигнальности). Визуальная иерархия: filled = приглушённая отметка, outline/accent = явный сигнал. Если semantic-консистентность важнее иерархии — приводить к одному hue (бэклог-тикет, не сделано). Зафиксировано 2026-05-29.
