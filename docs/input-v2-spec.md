...
**Толщина фокуса на Size sm (40).** Обсуждалось снижение focus/error-обводки до 1px на sm (2px пропорционально тяжелее на компактном поле). **Решение (2026-07-01): оставить 2px (`border/emphasis`) на всех размерах.** Причина: консистентность аффорданса важнее эстетики толщины — на Outline sm обводка 1px отличалась бы от дефолта только цветом, а заметность фокуса/ошибки критичнее именно на плотных экранах, где живёт sm; плюс единое размеро-агностичное правило проще для кода и спеки. Помечено на **валидацию в продукте** — при подтверждённом дискомфорте вернёмся к размеро-зависимой толщине.
...
> **`ReadOnly` ≠ поле-с-пикером (важно для сборки экранов и Code Connect).** `State=ReadOnly` (`Background/Tertiary`) — это **статичное поле-дисплей**: показывает зафиксированное значение, которое пользователь не редактирует и **не выбирает** (тап ничего не открывает). Приглушённый фон Tertiary — намеренный сигнал «неактивно».
> **Поле, по тапу которого открывается пикер/меню выбора** (валюта, единица, категория — напр. селектор `TT$` на «Цена услуги») — это **не Input и не ReadOnly, а компонент [`Select`](./select-spec.md)** (Field + шеврон → overlay). Select в покое живёт на chrome Field (`Background/Secondary`) — поэтому в макетах такое поле корректно выглядит «Filled/Secondary», а не Tertiary. Ошибка — собирать его инстансом `Input`; правильно — инстанс `Select`. См. [screen-assembly-spec §6](./screen-assembly-spec.md).
...
> **Реализация каретки в Focused (ответ дизайнера 2026-07-27, D1).** Figma-макет показывает `[каретка → зазор 2px → серый плейсхолдер]`. В коде достаточно **нативного поведения** платформы: каретка в позиции 0 (слева), плейсхолдер начинается там же. Пиксель-точный матч «каретка + зазор + плейсхолдер» **не требуется** — состояние транзиентное (исчезает при первом вводе), кастомный layout каретки не оправдан. Зазор 2px в Figma — визуальная деталь макета, не требование к рантайму.
