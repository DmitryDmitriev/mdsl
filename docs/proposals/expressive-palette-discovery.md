...
## 9. Applied-слой (held-цвета) — механизм токенизации expressive

**Решено с Дмитрием 2026-07-30. Ратифицировано в `COLOR-PALETTE.md` §2.12 (слой) + §3.9 (правило adaptive vs applied) — 2026-08-05.** Ключевая находка: у нас **уже есть** механизм theme-invariant цвета — суффикс **`applied`** (`White applied`, `Black applied` = один и тот же цвет в обеих темах, для on-color/over-media). Его надо **распространить на цветные оттенки** — это и есть недостающий «Expressive/held» слой (подход B §6), только под именем из нашей системы.

**Две роли цвета, которые раньше путали:**
| Роль | Пример | Поведение |
|---|---|---|
| Функциональная (адаптивная) | `Accent/Link` | светлеет в dark — для читаемости UI-акцентов |
| **Applied / held** (декор-идентичность) | коач-марк, tier-бейджи, expressive-плитки | **константа по теме**, среда подстраивается |

Ошибка была брать адаптивный `Accent/Link` для held-поверхности (в dark светлеет до `#60a5fa` → белый текст 2.5:1, провал). Held-цвет решает и «из библиотеки», и «контраст в обеих темах» разом (белый на `#2563eb` = 4.6:1 в обеих).

**Заведено в App Color Palette (коллекция Color, held = одинаково Light/Dark), первый кирпич applied-слоя (2026-07-30):**
- `Background/Blue applied` = алиас Blue/600 `#2563eb` (key `7010fec7a0d1c2d9c23a80a90c685e4f722b0224`) — expressive-поверхность.
