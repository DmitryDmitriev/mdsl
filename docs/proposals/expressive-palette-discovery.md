...
- **Открытые вопросы к Дмитрию:** (1) принимаем ли разделение на 2 трека (Tinted-лейблы vs Expressive-поверхности)? (2) какой подход к expressive — B (dual-token static) или C (overlay), или гибрид? (3) нужен ли category-hue per вертикаль (пока гипотеза, в макетах не материализовался)?
- **Память:** записать как structural DS-decision — **только после** выбора подхода (сейчас discovery, решения нет).
