...
**`Filled` (значение, не в фокусе).** Значение `Text&Icon/Primary`, обводка `Border/Default` (1px), каретки нет. **Иконка очистки × (`Right Icon`) остаётся видимой** — значение введено, очистка доступна без входа в фокус (LIOS-2526).
...
| **Right Icon** | **off** | Иконка очистки × (`ic_close` из Assets) — **on, когда есть значение: в `Editing` И `Filled`** (очистка доступна и без входа в фокус) |
...
> **Платформенная оговорка (iOS / UIKit) — LIOS-2526.** Описание `Caret` выше — про **Figma-представление** макета. На iOS используется **системная каретка** `UITextField`: задаётся только цвет (`tintColor = Text&Icon/Primary`), а ширина / высота / радиус — параметры макета, платформой **не воспроизводятся** (геометрию системной каретки UIKit не отдаёт; кастомная отрисовка означала бы ручной текстовый ввод и деградацию UX — лупа, выделение, drag). Android — аналогично (нативный курсор, тонируется). То есть спека-каретка = визуальный ориентир, не реализационное требование по геометрии.
...
**2026-08-12 — reconciliation с iOS (LIOS-2526, QA Ilia Karpenko).** Правки канона (раздел A дизайн-решений пуст — только уточнения):
- **B1** — явно зафиксировано: иконка очистки × (`Right Icon`) видима не только в `Editing`, но и в **`Filled`** (значение введено → очистка доступна без фокуса). Реализация уже корректна (`clearButton` виден при `.editing || .filled`).
- **C1** — добавлена платформенная оговорка к `Caret`: на iOS/UIKit каретка **системная**, тонируется `Text&Icon/Primary`; ширина/высота/радиус — параметры Figma-макета, платформой не воспроизводятся.
- a11y / тап-зона sm / Esc — починены в ветке `feature/LIOS-2526`, канон-правок не требуют.
