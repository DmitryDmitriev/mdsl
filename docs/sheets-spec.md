...
| **Низ (`pB`)** | **0** (`button=true`) / **20** (`button=false`) | единый baseline: нижний контент всегда на 32 от кромки (см. таблицу низа) |
...
**Низ — единый baseline, НЕ по типу контента** (пересмотр 2026-09-20-вечер: контент-зависимый низ давал «гуляющую» нижнюю линию — меню на 12, текст на 44, кнопка на 32 от кромки).

Правило: **нижний контент всегда садится на 32 от нижней кромки панели** — там, где стоит CTA-кнопка. Кнопка задаёт baseline, остальные типы к нему подтягиваются:

| Низ шторки | Как | Итог до кромки |
|---|---|---|
| **`button=true`** | CTA сам: Buttons Stack `pB=20` + root safe-area `12` | **32** ✅ (референс) |
| **`button=false`** (меню И текст, одинаково) | `slot pB = 20` (`spacing/5`) + root safe-area `12` | **32** |

- **Одинаково для меню и текста** — деления по типу больше нет (это и порождало разнобой).
- Низ живёт в ОДНОМ месте — `slot pB` (для button=false) или Button area (для button=true). На контент-обёртке (`inset-16`) нижнего паддинга НЕ держим.
- Почему `pB=20`, а не 16/24: baseline `32` задан геометрией кнопки (её `20 + 12`); `slot pB=20` подгоняет остальные ровно под неё.
- Оптическая компенсация: `List/Menu`-строка держит текст выше центра бокса, поэтому даже при равных боксах её последний текст читается чуть выше текста-параграфа — это норма, доводим глазом при необходимости.

Кейсы: меню `56698:6566` (пункт поднят 12→32), `Translate` `56698:42101` (текст опущен 44→32) — оба на одной линии с Submit из `Report`.

⚠️ Правки низа — на стороне композиции (`slot pB` для button=false), ре-паблиша не требуют.
...
3. **Низ — единый baseline `32` от кромки** (нижний контент везде на одной линии, где стоит кнопка):
   - **есть Button area** (`button=true`) → `slot pB = 0` (baseline даёт CTA: 20 + safe-area 12);
   - **нет кнопки** (меню И текст одинаково) → `slot pB = 20` (+ safe-area 12 = 32). На `inset-16` низ НЕ вешаем — только на слоте. (§2.5.2)
...
**2026-09-20 (вечер) — низ: единый baseline `32` от кромки вместо контент-зависимого.** Пересмотр дневного правила «низ по типу контента» (меню→12, текст→32): оно давало «гуляющую» нижнюю линию (меню 12 / текст 44 / кнопка 32 — три разных расстояния до кромки, видно направляющей `Y=1816`). Новый канон: нижний контент ВЕЗДЕ на 32 от кромки, как CTA-кнопка. `button=true` → `slot pB=0` (CTA: 20+12); `button=false` (меню И текст) → `slot pB=20` (+12 safe = 32). Убран `inset pB=32` у Translate. Прогнано по всем button=false в `PI3XrU`. §2.5.2. Compose-side. Кейсы: `56698:6566` (пункт 12→32), `56698:42101` (текст 44→32).
