...
|---|---|---|
| **Верх (`pT`)** | **16** (`section/padding-default`) | **слот** — осознанное исключение: Header даёт снизу всего 8, этого мало для отрыва заголовка от контента, поэтому верхний спейсер держит слот |
| **Низ (`pB`)** | **0** | **сосед снизу**, не слот: при `button=true` — Button area (свой `pT`); при `button=false` (меню) — safe-area 12 (паттерн «меню до кромки») |

**Почему не `24/24` (старый канон):** симметричные 24 задваивались снизу с `pT` у Button area (контент→кнопка = 24+20 = 44, «дыра») и сверху с `pB=8` Header'а. Асимметрия убирает оба задвоения.
...
⚠️ **Меню (`button=false`):** последний пункт окажется в 12px (safe-area) от кромки вместо прежних 36. Это стандартный паттерн, но если конкретному меню нужен воздух снизу — добавляется инсетом на стороне контента, не паддингом слота.
...
**2026-09-20 — канон вертикали слота: `24/24` → `16/0` (асимметрия).** Мастер Bottom Sheet (`4584:1115`) content slot: `pT 24→16`, `pB 24→0`. Причина — симметричные 24 задваивались снизу с Button area (контент→кнопка 44, «дыра») и сверху с Header `pB=8`. Основание — аудит всех 16 DS-шторок в `PI3XrU`: «Действия» в Отзывах дизайнер уже держал на `pB=0`. Верх остаётся на слоте (Header даёт мало), низ — на соседе (Button area / safe-area 12 для меню). §2.5.2. Требует ре-паблиша UI-Kit. См. также память `mds-bottom-sheet-vertical-canon`.
