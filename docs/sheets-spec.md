...
| Параметр | Токен / значение |
|----------|------------------|
| Padding вертикальный (на слоте) | stack-md (24 px) — сверху/снизу тела |
...
|---|---|---|
| **List Item, медиа, Divider, full-width-строки** | **как есть, full-bleed** (свои 16 уже внутри List Item) | не добавлять |
| **Текст, Textarea, кнопки, отдельные контролы** | обернуть в 16-инсет-фрейм (paddingLeft/Right = spacing/4) | 16 обёрткой |

Дефолтный контент слота — 16-инсет-фрейм (обычный текст из коробки padded; списки дизайнер кладёт full-bleed, заменяя дефолт). Вертикальные 24 живут на слоте и на full-bleed не влияют.
...
| Body padding vertical (на слоте) | stack-md (24 px) |
