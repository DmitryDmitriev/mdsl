...
- Требуется **re-publish UI-Kit-Mobile** (вручную). После — в продуктовых файлах BS-инстансы сдвинут контент на 16 внутрь (может поехать окружающая вёрстка); ручные обходы (inner-frame с padding) снять. Известный кейс — инстанс `56698:42073` (Translate message BS) в `PI3XrU`.
