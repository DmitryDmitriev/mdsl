...
- **Background Old/Overlay × 8** на декоративных backdrop-rectangles внутри `Bottom Sheet` mockup'а. `Background/Overlay` (или `Alpha Black/60`) пока **не подключён** в UI-Kit-Mobile из App Color Palette — это Foundation-level open task (см. §«История миграций»). Визуально корректно: dark scrim rgba(29,32,35) с opacity 60% Light / 80% Dark.
...
| `HandleContainer` | `4558:690` | Контейнер ручки. 2 варианта: `Type=Handler` (выезжающий с handle 40×4) / `Type=без handle` |
...
- **`Background/Overlay` / `Alpha Black/60`** не подключён в UI-Kit-Mobile. В Sheets осталось 8 backdrop-фактов на `Background Old/Overlay` (rgba(29,32,35) с opacity 60% Light / 80% Dark) — единственный оставшийся Old-токен. Подключить canonical equivalent из App Color Palette (где Alpha Black палитра живёт по `COLOR-PALETTE.md`). Это Foundation-level task — после его закрытия dialog backdrop + sheet backdrop + любой другой scrim придут к одному токену.
