...
- **Тип = семантика.** Каждый тип имеет свой `Background/Tinted/*` фон + соответствующую иконку.
...
| Type | Background | Title color | Description color | Supporting color | Icon (16 / 24 / 32) |
|---|---|---|---|---|---|
| **Info** | `Background/Tinted/Info` | `Text&Icon/Primary` | `Text&Icon/Primary` | `Text&Icon/Primary` | `ic_info` / `ic_info` / `ic_Info` |
| **Warning** | `Background/Tinted/Warning` | `Text&Icon/Primary` | `Text&Icon/Primary` | `Text&Icon/Primary` | `ic_warning_amber` / `ic_warning` / `ic_WarningCircle` |
| **Good** | `Background/Tinted/Good` | `Text&Icon/Primary` | `Text&Icon/Primary` | `Text&Icon/Primary` | `ic_check_circle` / `ic_check_circle` / `ic_CheckCircle` |
| **Tech** | `Background/Tinted/Neutral` | `Text&Icon/Primary` | `Text&Icon/Primary` | `Text&Icon/Primary` | `ic_info` / `ic_info` / `ic_Info` |
| **Negative** | `Background/Tinted/Negative` | `Text&Icon/Negative` | `Text&Icon/Negative` | `Text&Icon/Negative` | `ic_declined` / `ic_declined` / `ic_MinusCircle` |
...
```
Alert (HORIZONTAL, FIXED 328, padding=spacing/3 (12), gap=spacing/2 (8), radius=12, align MIN top)
├── icon-wrap (HUG, paddingTop=1 — оптическая компенсация иконки, design constant)
│   └── icon (16×16, ic_*)
...
Alert (HORIZONTAL, FIXED 328, padding=spacing/3 (12), gap=spacing/2 (8), radius=12, align MIN top)
├── icon (24×24, ic_*)
└── inner (VERTICAL, FILL, gap=spacing/1 (4), paddingTop=1 — оптическая компенсация text-блока)
...
```
Alert (VERTICAL, FIXED 328, padding=spacing/3 (12), gap=spacing/4 (16), radius=16, fill = tinted)
├── content-row (HORIZONTAL, FILL/HUG, gap=spacing/3 (12), align MIN top)
│   ├── icon (32×32, ic_*)
│   └── inner (VERTICAL, FILL, gap=spacing/2 (8), paddingTop=spacing/1 (4) — оптическая компенсация text-блока)
...
| Gap (root) | `spacing/2` (8) — icon↔text | `spacing/2` (8) — icon↔text | `spacing/4` (16) — content-row↔action |
| Gap (content-row) | — | — | `spacing/3` (12) — icon↔inner |
| Gap (inner) | — | `spacing/1` (4) — title↔desc | `spacing/2` (8) — title↔desc↔supp |
| Optical paddingTop | `1` (icon-wrap, design constant) | `1` (inner, design constant) | `spacing/1` (4) (inner) |
| Radius | `radius/3` (12) | `radius/3` (12) | `radius/4` (16) |
| Icon size | 16 × 16 | 24 × 24 | 32 × 32 |
| Border | нет (без обводки во всех 3 стилях) | нет | нет |
...
| Слой | Стиль | Цвет |
|---|---|---|
| Compact body | `Base/Body 2` (14/20) | `Text&Icon/Primary` (Negative: `Text&Icon/Negative`) |
| Standard title | `Base/Body 1 Medium` (16/24) — был Body 2 Medium, поднят чтобы балансировать с 24-иконкой | `Text&Icon/Primary` (Negative: `Text&Icon/Negative`) |
| Standard description | `Base/Body 2` (14/20) | `Text&Icon/Primary` (Negative: `Text&Icon/Negative`) |
| Card title | `Heading/H4 Bold` — был Body 1 Medium, поднят до H4 Bold чтобы балансировать с 32 fill-иконкой | `Text&Icon/Primary` (Negative: `Text&Icon/Negative`) |
| Card description | `Base/Body 2` (14/20) | `Text&Icon/Primary` (Negative: `Text&Icon/Negative`) |
| Card supporting | `Caption/caption-md` (12/16) | `Text&Icon/Primary` (Negative: `Text&Icon/Negative`) |

**Принцип:** иерархия задаётся **типографикой** (Bold 16 → Regular 14 → Regular 12), не цветом. `Text&Icon/Tertiary` не применяется — на тёмных tinted-фонах (Dark theme, Green/800, Blue/800…) даёт слабый контраст.
...
**2026-06-05 (вечер) — supporting text color зафиксирован как `Text&Icon/Primary`.**

Пробел в спеке: §3 и §7 не указывали цвет для Supporting слоя Alert Card. iOS-код использовал `Text&Icon/Tertiary` как дефолт. Определён канон — **`Text&Icon/Primary`** (Negative: `Text&Icon/Negative`), по той же логике, что Title и Description:

- Иерархия в Alert — через типографику (Bold 16 → Regular 14 → Regular 12), не цвет.
- `Text&Icon/Tertiary` (Zinc/300 Light, Zinc/600 Dark) даёт слабый контраст на тёмных tinted-фонах Dark mode (Green/800, Blue/800, Orange/800…) — ровно та проблема, из-за которой 2026-05-04 текст в Alert ушёл от серых к Primary.
- `Text&Icon/Secondary` — промежуточный вариант, избыточен: иерархия уже задана размером 12.

Figma master исправлен: 5 нод `supporting` (6947:14065/14080/14093/14108 → `Text&Icon/Primary`; 6947:14122 Negative → `Text&Icon/Negative`). iOS-fix: `Token.Color.TextIcon.tertiary → Token.Color.TextIcon.primary` (`.negative` для Type=Negative). §3 расширен столбцом Supporting color, §7 дополнен колонкой Цвет + примечание про принцип.
...
- **`itemSpacing = 10`** (off-scale, наследие M3) на root-уровне Compact и Standard вариантов → заменён на **`spacing/2` (8)**. Закрыта последняя off-scale дыра в компоненте.
- **`strokeWeight = 1`** на 35 фреймах (root, content-row, inner и др.) — был hardcoded без видимого бордера. Alert **не имеет обводки** в любом стиле → strokeWeight убран (0).
- **`Style=Compact, Type=Negative` icon Vector** имел hardcoded `#000000` → привязан к `Text&Icon/Negative`.
- **`paddingTop = 4`** на `inner` фреймах Card-вариантов → привязано к `spacing/1`.
- **Восстановлены биндинги** boolean пропов `Description` и `Supporting` на text-нодах (раньше 0 биндингов — пропы существовали впустую). Теперь 10 + 5 = 15 биндингов соответствуют 5 Card + 5 Standard variants.
- **`paddingTop = 1`** на icon-wrap (Compact) и inner (Standard) — оставлены как design constant (оптическая компенсация по §11.1 DESIGN-TOKENS).

Финальный аудит: 100% видимых цветов и numeric props на токенах, кроме разрешённой оптической компенсации (9× paddingTop=1).
...
Возвращаем семантику: Type=Good → **`Background/Tinted/Good`** (Green/50, `#f0fdf4` Light / `#166534` Dark). Принимаем чуть более бледную плашку как корректный канон — текст `Text&Icon/Primary` контрастен и на Green/50. (Old shade `Background/Tinted/Admin` (Green/100) позже удалён из канона 2026-05-07; admin-зелёный остался только в легаси-группе `Decor/Bubble Old/Admin` за чат-баблами.)
...
> ⚠️ **Обновлено 2026-05-11:** значение `10` устарело — приведено к `spacing/2` (**8 px**) в рамках финальной чистки off-scale значений. Текущий канон в Figma master и в таблицах §4/§6 — **8 px** для Compact и Standard root. См. запись «2026-05-11 — финальная чистка токенов и off-scale значений» выше.
