...
15 вариантов (5 type × 3 style). Hardcoded `paddingTop=2` в `icon-wrap` (Compact) — оптическая компенсация выравнивания 16-иконки с первой строкой Body 2 текста. Намеренное design-decision, не баг.
...
1. Все иконки заменены на `ic_*` варианты (новая конвенция в библиотеке) — `16/ic_info`, `16/ic_warning_amber`, `16/ic_check_circle`, `16/ic_declined_fill`; `24/ic_info`, `24/ic_warning`, `24/ic_check_circle`, `24/ic_declined`; `32/ic_info_fill`, `32/ic_warning_fill`, `32/check_circle_fill_fill`, `32/ic_declined_fill`. **Convention:** для всех новых компонентов использовать только иконки с `ic_` префиксом.
