...
**Коллекция в Figma:** «Spacing — usage» — node `9119:10` в UI-Kit-Mobile (`PI2N65xbeJPTc5oWhOP7Bl`). 20 токенов, все алиасят на Core primitives. У каждого — русское описание в Figma description (кодирует модель A/B + ось, см. ниже).

**Primitives `spacing/*` скрыты из меню padding/gap** (scope без `GAP`) — биндить отступы можно только семантикой, primitives остаются источником алиасов. Это убирает core-значения из выпадашки при бинде отступа.

**Cheatsheet:** node `9121:3651` (детальная таблица токенов). **Визуальная схема выбора** (модель A/B + флоу) — на странице-доке `4334:52`, фрейм «Spacing — какой токен выбрать».

---

## Модель — два семейства

Токены отступов — **не иерархия** (`screen → section → stack → row` так не работает), а две независимые семьи:

- **A · Контейнерные — «где?»** Привязаны к типу контейнера: `screen/*` (край экрана), `section/*` (section card: padding внутри, gap между картами, заголовок→контент), `list/*` (строки списка), `chip/*` (внутри chip), `cta/*` (нижняя CTA-панель).
- **B · Направленные — «как между соседними элементами?»** По оси: `stack/*` — **вертикаль** (колонка: Title↕Subtitle, поля формы), `row/*` — **горизонталь** (ряд: иконка+label, чипы).

Мнемоника: `stack` = вертикаль, `row` = горизонталь; остальное — по контейнеру. Карты разделяет `section/gap`, **не** `stack`.
