...
### Использование как value-якорь в Slider (2026-09-25, LIOS-2792)

Slider переиспользует этот же `Tooltip` `11122:21` для показа значения над ручкой при перетаскивании. Отличия от общих правил Tooltip:
- **Якорь** — активная ручка Slider; offset `8` (`spacing/2`) считается от верхнего края **увеличенной (24)** ручки Dragging (не от 20).
- **Одна строка с обрезкой** («…»), а НЕ перенос по `maxWidth 240`: значение (цена) в две строки читается плохо. Оверрайд переноса — только для value-якоря Slider; общий Tooltip переносит как прежде.
- Полное поведение (когда показывается, что показывает, анимация) — в [slider-spec.md](./slider-spec.md) §Поведение → Value tooltip.
