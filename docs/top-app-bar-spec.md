...
Итого **12 вариантов** = 3 × 2 × 2.

Leading — отдельный variant axis (а не Boolean), потому что у `Leading=No` другой `root.paddingLeft` (16 vs 4) — нужны разные layout'ы для соответствия Material Design edge padding в обоих случаях.
...
Leading=Yes: root.paddingLeft = 4 (иконка Leading визуально в 16 от края)
Leading=No:  root.paddingLeft = 16 (контент в 16 от края)
...
| Bar width | FILL | — |
| Padding top / bottom / right | 4 | `spacing/1` |
...
| **Padding (Leading=Yes, all sides)** | `spacing/1` (4) |
