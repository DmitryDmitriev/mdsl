...
- **Scrollable.** Сегменты по контенту (`HUG`) с **минимальной шириной `88` dp** (= 56 dp text-area + 2×16 dp padding H). Короткие подписи дают ровно 88 dp (совпадает с Figma), длинные — раздвигают свой сегмент дальше (без обрезки/ellipsis). Контейнер скроллится горизонтально. Используется когда сегментов 5+ или подписи длинные.

  > **Почему min-width + HUG, а не equal-width.** Equal-width (все сегменты по самому широкому) — это паттерн **Fixed**; если нужна равная ширина, берётся Fixed. Жёсткие 88 dp с ellipsis обрезали бы реальный контент («Real estate»). `min-width 88 + HUG` = M3-канон scrollable tabs: коротким сегментам даёт аккуратный минимум и совпадение с Figma, длинным — не ломает текст.
...
**Между соседними сегментами explicit gap нет** (`flex`, без `gap-N`) — ни в Fixed, ни в Scrollable. Визуальное разделение даёт собственный `Padding H` каждого сегмента (16 + 16 = 32 dp). Вертикальных сепараторов-линий между сегментами тоже нет.

**Стартовый отступ контейнера = 0** в обоих режимах. Первый сегмент стоит флешом к краю контейнера (его собственный `Padding H` 16 даёт отступ до текста), бар выравнивается по контентной сетке экрана. (В Figma на Scrollable-вариантах присутствует `pl-52` — это presentation-артефакт фрейма, в продукт **не тащить**.)
...
| **Inter-segment gap** | 0 (нет) | 0 (нет) |
| **Container start-padding** | 0 | 0 |
| **Min-width сегмента (Scrollable)** | 88 (HUG сверху) | 88 (HUG сверху) |

---

## Радиусы

В Figma на корневом контейнере и индивидуальных сегментах `cornerRadius = 0`. Сегменты — плоские, с нижним indicator (pill-top 3 px) для Primary и заливкой для Secondary. Вертикальных сепараторов-линий между сегментами нет — разделение чисто через `Padding H` каждого сегмента.
...
- **Disabled.** Управляется на родителе: `aria-disabled="true"` + `pointer-events: none` + `opacity: 0.4`. На самом сегменте отдельных Disabled-вариантов нет. **Tap-события полностью игнорируются: ни ripple/state-layer, ни callback — «глухой» элемент** (Android: `clickable(enabled = false)`; iOS: `isUserInteractionEnabled = false`). Промежуточного «ripple без смены selection» не делаем — это противоречит a11y-семантике disabled.
...
--segment-min-width-scroll: 88px;                       /* Scrollable: HUG сверху, min 88 */
--segment-gap-between:      0;                           /* нет gap между сегментами */
--segment-start-padding:    0;                           /* контейнер флешом к краю */
...
**2026-06-18 — QA-reconciliation LAA-3772 раунд 2, 3 уточнения (Android implementation).**

- **#6 — ширина сегментов в Scrollable.** Канон = **HUG с min-width 88 dp** (вариант C). Не equal-width (это паттерн Fixed) и не чистый HUG. Короткие подписи = 88 dp (совпадает с Figma), длинные раздвигают сегмент без ellipsis. §Type Fixed vs Scrollable дополнена явной формулировкой.
- **#7a — inter-segment gap.** Подтверждено: explicit gap между сегментами **нет** (ни Fixed, ни Scrollable); разделение через собственный `Padding H` 16+16. Вертикальных сепараторов-линий тоже нет — убрана ошибочная строка из §Радиусы про `border-left`-сепараторы. Зафиксировано в §Структура слоёв + §Размеры.
- **#7b — `pl-52` на Scrollable-контейнере.** Это **presentation-артефакт** Figma-фрейма, НЕ канон. Стартовый отступ контейнера = 0, бар выравнивается по контентной сетке экрана. В продукт не тащить.
- **#8 — Disabled ripple.** Канон = **(A) без ripple, без callback** — «глухой» элемент. Промежуточный вариант «ripple без смены selection» отклонён (противоречит a11y). §A11y → Disabled дополнена.
