...
**Ключевое решение по Active (финал 2026-05-26).** Active=Yes отличается от Active=No **двумя** изменениями одновременно: (1) фон смещается Background/Tertiary → **Background/Secondary** (на 1 ступень в светлую сторону в light, в тёмную — в dark; малозаметный, но различимый сдвиг), (2) добавляется **1px solid `Border/Active` (INSIDE)**. Текст/иконка остаются `Text&Icon/Primary` (без инверсии).

Промежуточные варианты, которые рассматривались и были отброшены:
- **Только 1px Border/Active без смены fill** — оказался визуально слабым (border теряется на Icon-Only chip'е) → добавили сдвиг fill.
- **Inverted fill (`Background/Inverted Primary` + `Text&Icon/Inverted W-B`)** — слишком сильный сигнал, ломал визуальный ритм filter-card'а и плохо ложился на dropdown-паттерн (где текст лейбла уже сигнализирует «выбрано»). Откат 2026-05-26.
...
4. **Active=Yes — fill shift + bright border** (финал после нескольких итераций): `Background/Tertiary` + 1px `Border/Active` → **`Background/Secondary` + 1px `Border/Active`**. Текст/иконка остаются `Text&Icon/Primary`. Итерации: исходный variant (только border) → inverted (Background/Inverted Primary + Inverted W-B, отброшен как «слишком кричит и ломает dropdown-паттерн») → откат к neutral визуально идентичному Active=No (отброшен как «слабый сигнал») → финал: fill смещается на одну ступень + bright border. Subtle, но различимо. Меняет визуал ВСЕХ Active=Yes чипов, не только Icon-Only.
