...
|---|---|---|---|---|
| **Filled** (default) | **No** | `Background/Tertiary` | — | `Text&Icon/Primary` |
| **Filled** | **Yes** | `Background/Secondary` | — | `Text&Icon/Primary` |
...
- **Filled:** fill смещается Tertiary → Secondary (на одну ступень в сторону Primary). Subtle, без border'ов.
- **Outline:** fill Primary → Secondary + border меняется Default → Active. Два признака одновременно — сильнее читается, чем filled.

**Ключевое решение по элевации (Filled).** Active=No fill = `Background/Tertiary` (Zinc/100 Light, Zinc/800 Dark) — 2-шаговая элевация от `Background/Primary` страницы. В Dark с Secondary chip сливался бы с card-фоном.
...
4. **Fill axis (Filled / Outline)** — как у Input и Badge:
   - **Filled, Active=No:** `Background/Tertiary`, без border.
   - **Filled, Active=Yes:** `Background/Secondary`, без border (subtle fill shift).
