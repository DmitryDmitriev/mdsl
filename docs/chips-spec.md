...
|---|---|---|---|
| **Active=No** (default) | `Background/Primary` | 1px `Border/Default` (INSIDE) | `Text&Icon/Primary` |
| **Active=Yes** (configured) | `Background/Secondary` | 1px `Border/Active` (INSIDE) | `Text&Icon/Primary` |

**Ключевое решение по визуальной модели (финал 2026-05-26).** Chip — это **outlined**-элемент в обоих состояниях. Default = «прозрачный» фон страницы (`Background/Primary`) + нейтральный border. Active = subtle fill shift на одну ступень (`Background/Secondary`) + bright `Border/Active`. Текст/иконка остаются `Text&Icon/Primary` в обоих состояниях.

Почему outlined-модель (не filled):
- **Симметрия default ↔ active.** Оба состояния — outlined. Active отличается двумя признаками одновременно (fill+border), оба различимы по отдельности, но вместе дают clear signal без визуального крика.
- **Работает в обеих темах.** В light: white-chip с тонким border на off-white фоне страницы. В dark: тёмный chip с тонким border на ещё более тёмном фоне. Border несёт элевацию, fill — лёгкий контраст.
- **Совместимо с цветной картинкой / image-фоном.** Chip визуально отделён от фона border'ом независимо от того, что под ним.

Промежуточные варианты, которые рассматривались и были отброшены:
- **Active=No filled (`Background/Tertiary`) без border + Active=Yes только border** — Active терялся, особенно на Icon-Only.
- **Active=Yes inverted (`Background/Inverted Primary` + `Text&Icon/Inverted W-B`)** — слишком сильный сигнал, ломал визуальный ритм filter-card'а и плохо ложился на dropdown-паттерн.
- **Active=Yes визуально идентичен Active=No** — слабый сигнал, нечитаемо.
...
4. **Outlined-модель для обоих состояний** (финал после нескольких итераций):
   - **Active=No:** `Background/Tertiary` (filled) → **`Background/Primary` + 1px `Border/Default`** (outlined).
   - **Active=Yes:** `Background/Tertiary` без border → **`Background/Secondary` + 1px `Border/Active`** (outlined, fill на ступень темнее, bright border).

   Итерации Active=Yes: только border (слабо) → inverted fill+text (кричит, ломает dropdown) → визуально идентичен Active=No (нечитаемо) → fill shift + bright border (финал). Затем — на той же сессии — Active=No тоже переведён на outlined для симметрии и для лучшей работы на image-фонах / в светлой теме (white-chip на off-white странице). Меняет визуал ВСЕХ chip'ов, не только Active=Yes.
