...
| **Text & longer action** | VERTICAL stack | Supporting text сверху, Action button + опц. Close снизу | Длинный текст не помещается рядом с action на одной строке → action переносится на отдельную строку под текстом |
...
├── Supporting text (TEXT) — Base/Body 2 (Inter 14/20 Regular)
├── Action button (INSTANCE: Button sm Ghost Inverted) — опц., 4-line иконка/текст
└── Close icon (INSTANCE: ButtonIcon Ghost Inverted, 24×24) — опц., при Show close affordance=True
```
...
| Padding (general) | 12/16 vertical/horizontal | `spacing/3` / `spacing/4` |
| Padding (compact w/ close) | 0 на нужной стороне (Close button самостоятельно даёт 16 px touch) | — |
| Gap text↔action | 4 px | `spacing/1` |
| Action button | sm (40 px) | `control-height/sm` (через инстанс) |
| Close icon | 24 × 24 | `size/sm` (через ButtonIcon-инстанс) |
...
|---|---|---|
| Supporting text | `Base/Body 2` | Inter 14/20 Regular |
...
- **Close icon:** `aria-label="Закрыть"`, размер touch-зоны 44×44 (внутренние 24×24 иконки + padding).
...
|---|---|
| 🎨 Color (fills) | **100%** |
| 🔤 Typography | **100%** (`Base/Body 2`, `Base/Body 2 Medium`) |
| 🌑 Elevation | **100%** (`Elevation/Floating`) |
| 🔲 Spacing / Radius | **100%** (`spacing/*`, `radius/1`, `control-height/*`, `size/sm`) |
| **Overall** | **100%** |
